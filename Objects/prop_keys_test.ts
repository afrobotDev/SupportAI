...
import { BIOMETRIC_LOCK, TWO_FACTOR, isSecure } from "./prop_keys.ts";
import type { MailPreferences } from "./prop_keys.ts";
...
describe("isSecure", () => {
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    it(`Test ${i}`, () => {
      ...
      // Also log symbol-keyed properties
      for (const sym of Object.getOwnPropertySymbols(preferences)) {
        console.log(`  - ${String(sym)}: ${preferences[sym as any]}`);
      }
